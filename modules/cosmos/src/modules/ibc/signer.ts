...
import { AccountData, DirectSignResponse, KdfConfiguration, makeSignBytes, OfflineDirectSigner } from "@cosmjs/proto-signing";
import { decrypt, encrypt, EncryptionConfiguration, executeKdf, supportedAlgorithms } from "@cosmjs/proto-signing/build/wallet";
...
/**
 * Derive Ethereum-style address from secp256k1 public key for Ethermint chains.
 * @param pubkey A compressed secp256k1 public key.
 * @returns The raw address bytes (20 bytes).
 */
function ethermintPubkeyToRawAddress(pubkey: Uint8Array): Uint8Array {
    if (pubkey.length !== 33) {
        throw new Error("Public key must be compressed secp256k1 (33 bytes)");
    }

    // Convert compressed pubkey to uncompressed
    const uncompressed = Secp256k1.uncompressPubkey(pubkey);

    // Remove the 0x04 prefix byte, keep only the 64-byte coordinates
    const pubkeyWithoutPrefix = uncompressed.slice(1);

    // Apply keccak256 hash and take the last 20 bytes for Ethereum address
...
export class DirectSecp256k1HdWallet implements OfflineDirectSigner {
     ...
     */
    static async fromMnemonic(mnemonic: string, options: Partial<DirectSecp256k1HdWalletOptions> = {}): Promise<DirectSecp256k1HdWallet> {
     ...
     */
    static async deserialize(serialization: string, password: string): Promise<DirectSecp256k1HdWallet> {
        const root = JSON.parse(serialization);
        if (!isNonNullObject(root)) throw new Error("Root document is not an object.");
        switch ((root as any).type) {
            case serializationTypeV1:
     ...
     */
    static async deserializeWithEncryptionKey(serialization: string, encryptionKey: Uint8Array): Promise<DirectSecp256k1HdWallet> {
        const root = JSON.parse(serialization);
        if (!isNonNullObject(root)) throw new Error("Root document is not an object.");
        const untypedRoot: any = root;
        switch (untypedRoot.type) {
            case serializationTypeV1: {
                const decryptedBytes = await decrypt(fromBase64(untypedRoot.data), encryptionKey, untypedRoot.encryption);
    ...
    private static async deserializeTypeV1(serialization: string, password: string): Promise<DirectSecp256k1HdWallet> {
        const root = JSON.parse(serialization);
        if (!isNonNullObject(root)) throw new Error("Root document is not an object.");
     ...
     */
    async serializeWithEncryptionKey(encryptionKey: Uint8Array, kdfConfiguration: KdfConfiguration): Promise<string> {
    ...
    private async getAccountsWithPrivkeys(): Promise<readonly AccountDataWithPrivkey[]> {
        return Promise.all(
            this.accounts.map(async ({ hdPath, prefix }) => {
                ...
                // Use Ethereum-style address derivation for Ethermint chains
                const addressBytes = ethermintPubkeyToRawAddress(pubkey);
